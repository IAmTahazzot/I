			...
			window.addEventListener('mousemove', (e: MouseEvent): void => {
				if (
					...
					(e.target instanceof Element &&
						e.target.hasAttribute('data-smart-cursor'))
				) {
					grow = true;
				} else {
					grow = false;
...
<svg
	class="w-full h-screen top-0 left-0 pointer-events-none z-[1000] hidden md:fixed"
...
<div
	class={cn(
		...
		!grow && 'bg-white',
		grow &&
			'border border-black dark:border-white scale-[200%] mix-blend-normal',
